...
document.addEventListener('DOMContentLoaded', async () => {
    try {
        ...
        if (!userDoc.exists) {
            ...
            alert("Seu usuário não está configurado corretamente. Redirecionando para a página de configuração.");
        ...
        alert("Erro ao carregar a aplicação. Por favor, recarregue a página ou entre em contato com o suporte.");
...
async function loadTasks() {
    try {
        ...
        try {
            ...
            const snapshotNoOrder = await db.collection('tasks')
                .where('userId', '==', currentUser.id)
                .get();
...
async function updateTask(taskId, updatedTask) {
    try {
        ...
        const taskUpdateData = {
            ...
            ...(updatedTask.responsible !== undefined && { responsible: updatedTask.responsible || currentTask.responsible || '' }),
            ...(updatedTask.startDate !== undefined && { startDate: updatedTask.startDate || '' }),
            ...(updatedTask.endDate !== undefined && { endDate: updatedTask.endDate || '' }),
            ...(updatedTask.subtasks !== undefined && { subtasks: updatedTask.subtasks || [] }),
            ...(updatedTask.status !== undefined && { status: updatedTask.status || 'in-progress' }),
            ...(updatedTask.completed !== undefined && { completed: updatedTask.completed }),
            updatedAt: firebase.firestore.FieldValue.serverTimestamp()
        };
        
        // Se apenas estamos atualizando as subtarefas, usamos update com o campo específico
        if (Object.keys(updatedTask).length === 1 && updatedTask.subtasks !== undefined) {
...
async function deleteTask(taskId) {
    try {
        ...
        const deleteBtn = document.querySelector(`.task-card[data-id="${taskId}"] .delete`);
        if (deleteBtn) {
            const originalContent = deleteBtn.innerHTML;
            deleteBtn.innerHTML = '<span class="material-symbols-outlined loading-spin">sync</span>';
...
function createTaskElement(task) {
    ...
    taskCard.innerHTML = `
        ...
        ${task.responsible ? `<div class="task-responsible"><strong>Responsável:</strong> ${task.responsible}</div>` : ''}
...
function createCollapsedSubtaskElementFromData(subtask) {
    ...
    removeBtn.addEventListener('click', () => {
        ...
        showConfirmModal(`Tem certeza que deseja excluir esta subtarefa?`, () => {
            ...
            removeBtn.innerHTML = '<span class="material-symbols-outlined loading-spin">sync</span>';
        ...
        input.addEventListener('change', async () => {
            if (editingTask && editingTask.id) {
                ...
                try {
                    ...
                    await saveSubtaskChanges(editingTask.id, updatedSubtasks);
                    
                    // Mostrar indicador de sucesso
                    showSaveIndicator('Subtarefa atualizada com sucesso!', 'success');
                    
                    // Recarregar tarefas e atualizar UI completa
                    await loadTasks();
                    renderTasks();
                    
                    // Se a visualização de subtarefas estiver aberta, atualizar
                    if (!subtasksView.classList.contains('hidden')) {
                        // Remover todas as subtarefas atuais antes de renderizar novamente
                        allSubtasksContainer.innerHTML = '';
                        renderAllSubtasks();
                    }
                } catch (error) {
                    console.error('Erro ao atualizar subtarefa:', error);
                    showSaveIndicator('Erro ao atualizar subtarefa', 'error');
                }
            }
        });
    });
        ...
        showConfirmModal(`Tem certeza que deseja excluir a subtarefa "${title}"?`, () => {
            ...
            setTimeout(() => {
                ...
                if (editingTask && editingTask.id) {
                    ...
                    saveSubtaskChanges(editingTask.id, updatedSubtasks)
                        .then(async () => {
                            // Recarregar tarefas e atualizar listas
                            await loadTasks();
                            renderTasks();
                        ...
                        .catch(error => {
                            ...
                            showSaveIndicator('Erro ao excluir subtarefa', 'error');
                        });
                }
            }, 300);
        });
...
function renderAllSubtasks() {
    ...
    if (subtasksView.classList.contains('hidden') || subtasksView.dataset.isClosing === "true") {
        return;
    }
    ...
    if (filteredSubtasks.length === 0) {
        ...
        emptyState.innerHTML = '<p>Nenhuma subtarefa encontrada com os filtros selecionados.</p>';
...
function createAllSubtaskElement(subtask) {
    ...
    element.innerHTML = `
        ...
        <div class="all-subtask-details">
            ...
            <div class="all-subtask-status ${subtask.status}">
                <span class="material-symbols-outlined">
                    ${subtask.status === 'completed' ? 'check_circle' : 
                      subtask.status === 'partial' ? 'incomplete_circle' : 
                      subtask.status === 'no-response' ? 'help' : 'pending'}
                </span>
                <span>${statusText}</span>
            </div>
            <div class="all-subtask-comm-type ${subtask.communicationType || 'enviado'}">
                <span class="material-symbols-outlined">
                    ${subtask.communicationType === 'recebido' ? 'call_received' : 'call_made'}
                </span>
                <span>${subtask.communicationType === 'recebido' ? 'Recebido' : 'Enviado'}</span>
            </div>
        </div>
        ${subtask.description ? `
    ...
    deleteButton.addEventListener('click', (e) => {
        ...
        showConfirmModal('Tem certeza que deseja excluir esta subtarefa?', async () => {
            try {
                const taskId = subtask.parentTaskId;
                const task = tasks.find(t => t.id === taskId);
...
function setupEventListeners() {
    ...
    const filterInputs = document.querySelectorAll('#subtask-title-filter, #subtask-description-filter');
    const filterSelects = document.querySelectorAll('#parent-task-filter, #status-filter, #comm-type-filter');
    ...
    const taskFilter = document.getElementById('task-filter');
    if (taskFilter) {
        taskFilter.addEventListener('input', debounce(() => {
            renderTasks();
        }, 300));
    }
...
function handleTaskFormSubmit(e) {
    ...
    submitBtn.innerHTML = '<span class="material-symbols-outlined loading-spin">sync</span> Salvando...';
    ...
    const taskData = {
        ...
        status: document.getElementById('task-status').value === 'true' ? 'completed' : 
                (document.getElementById('task-status').value === 'partial' ? 'partial' : 'in-progress'),
