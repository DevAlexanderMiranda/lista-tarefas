                ...
                db.collection('users').doc(user.uid).get()
                    .then(doc => {
                        if (doc.exists) {
                            ...
                            console.warn('Usuário autenticado mas sem dados no Firestore - Criando documento automático');
    ...
    try {
        ...
        if (doc.exists) {
            ...
            try {
                ...
                console.error('Erro ao criar documento de usuário durante login:', docError);
                return {
                    id: user.uid,
                    email: user.email,
                    name: user.displayName || user.email,
...
async function logout() {
    try {
        ...
        console.log('Fazendo logout do usuário:', currentUser ? currentUser.email : 'desconhecido');
...
async function createUser(userData) {
    try {
        ...
        const currentUser = await getCurrentUser();
        console.log('Usuário atual antes de criar:', currentUser ? currentUser.email : 'nenhum');
        ...
        if (!adminEmail || !adminPassword) {
            console.error('Credenciais de administrador não encontradas para reautenticação');
        }
        ...
        try {
            ...
            if (currentLoggedUser && currentLoggedUser.uid !== adminUid) {
                console.log('Usuário foi alterado para o novo criado, fazendo reautenticação...');
            ...
            console.error('Erro ao criar usuário com método alternativo:', innerError);
            throw innerError;
        }
    } catch (error) {
